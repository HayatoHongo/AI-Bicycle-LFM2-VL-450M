...
# -*- coding: utf-8 -*-
"""
Fine-tune LiquidAI/LFM2-VL-* on English text-only multi-turn chat data (.jsonl version).
- uses "content" field
- only the last assistant turn is trained (on-last)
- multi_modal_projector / vision_tower are frozen
"""
...
class EnChatTextOnlyOnLastDataset(Dataset):
    def __init__(self, json_path, system_message="Respond naturally in English.", split_ratio=0.98, split="train", seed=42):
        super().__init__()
...
# ================== Collator (on-last masking) ==================

def create_onlast_mask_collate_fn(processor):
    def collate_fn(samples):
        ...
        for i in range(input_ids.size(0)):
            ...
            # find all positions of <|im_start|>assistant
            starts = [j for j in range(len(ids) - 1)
                      if ids[j] == im_start_id and ids[j + 1] == assistant_id]

            # mask everything except the last assistant block
            if len(starts) == 0:
                labels[i, :] = -100
                continue
            last_start = starts[-1]
            labels[i, : last_start + 2] = -100  # mask up to last assistant start
...
def parse_args():
    ...
    p.add_argument("--output_dir", type=str, default="./output_onlast")
    ...
    p.add_argument("--system_message", type=str, default="Respond naturally in English.")
...
def main():
    ...
    print("📋 ===== Text-only On-Last Training Configuration =====")
    ...
    # === Dataset ===
    train_ds = EnChatTextOnlyOnLastDataset(args.chat_json, system_message=args.system_message, split="train", seed=args.seed)
    eval_ds = EnChatTextOnlyOnLastDataset(args.chat_json, system_message=args.system_message, split="eval", seed=args.seed)

    print(f"✅ Train samples: {len(train_ds)}")
    print(f"✅ Eval  samples: {len(eval_ds)}")

    collate_fn = create_onlast_mask_collate_fn(processor)
